...
BALANCE_PID_KP     = 10.00
BALANCE_PID_KI     = 120.00
BALANCE_PID_KD     =  0.05
BALANCE_PID_TARGET =  2.5
        ...
        for event in gamepad.read_loop():
            if event.type == ecodes.EV_KEY:
                if event.value == 1:
                    ...
                    elif event.code == L_TRIG_PUSH:
                        if debug_mode == True:
                            print("left trigger pushed")

                    elif event.code == R_TRIG_PUSH:
                        if debug_mode == True:
                            print("right trigger pushed")

                    elif event.code == START_BTN:
                        if debug_mode == True:
                            print("start")

                    elif event.code == SELECT_BTN:
                        if debug_mode == True:
                            print("select")

                    elif event.code == LB_BTN:
                        if debug_mode == True:
                            print("button LB")

                    elif event.code == RB_BTN:
                        if debug_mode == True:
                            print("button RB")

            if event.type == ecodes.EV_ABS:

                    if event.code == LT_TRIG:
                        if debug_mode == True:
                            print("left trigger: " + str(event.value))

                    elif event.code == RT_TRIG:
                        if debug_mode == True:
                            print("right trigger: " + str(event.value))

#MIDDLE 32768

# LEFT 0 <-- XAXIS --> 65535 RIGHT

#                    elif (event.code == X_AXIS) and (-5000 < event.value): # LEFT 0 <-(MIDDLE 32768)-> 65535 RIGHT
#                        if event.value > 5000:
#                            turn_angle_order = 0.0
#                            turn_speed_step  = 0.0
#                            target_speed     = TARGET_SPEED_STEP  * (event.value - MIDDLE_VALUE) / 30000
...
#                        if debug_mode == True:
#                            print("Moving forward | x axis: " + str(event.value))



                    elif (event.code == Y_AXIS) and (event.value > 5000): #FORWARD 0 <-(YAXIS MIDDLE 32768)-> 65535 BACKWARD
                        if event.value > 5000:
                            turn_angle_order = 0.0
                            turn_speed_step  = 0.0
                            target_speed     = TARGET_SPEED_STEP  * (event.value - MIDDLE_VALUE) / 10000
                        ...
                        if debug_mode == True:
                            print("Moving forward | y axis: " + str(event.value))

                    elif event.code == Z_AXIS:
                        if debug_mode == True:
                            print("z axis: " + str(event.value))

                    elif event.code == RZ_AXIS:
                        if debug_mode == True:
                            print("rz axis: " + str(event.value))
